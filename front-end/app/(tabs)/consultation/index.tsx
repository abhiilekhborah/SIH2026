...
export default function PatientConsultationIndex() {
  ...
  const handleJoin = async () => {
    ...
    if (!trimmedId) {
      Alert.alert("Enter Consultation ID", "Please enter the consultation ID shared by your doctor.");
      return;
    }

    setLoading(true);
    try {
      const userName = user?.firstName || user?.fullName || "Patient";
      const data = await requestConsultationToken(trimmedId, "patient", userName);

      router.push({
        pathname: "/(tabs)/consultation/active-call" as any,
        params: {
          token: data.token,
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.title}>Join a Consultation</Text>
        <Text style={styles.subtitle}>
          Enter the consultation ID provided by your doctor to start a voice or
          video call.
        </Text>
        ...
        <View style={styles.infoCard}>
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoText}>Voice call starts automatically</Text>
          </View>
          <View style={styles.infoRow}>
            <Ionicons name="videocam-outline" size={18} color="#64748B" />
            <Text style={styles.infoText}>Video can be enabled during call</Text>
          </View>
          <View style={styles.infoRow}>
            <Ionicons name="wifi-outline" size={18} color="#64748B" />
            <Text style={styles.infoText}>Optimized for low-bandwidth connections</Text>
          </View>
        </View>
      </View>
    </SafeAreaView>
