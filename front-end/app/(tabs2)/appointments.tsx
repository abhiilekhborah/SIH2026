...
export default function Appointments() {
  ...
  const dayAppointments = useMemo(() => {
    return appointments.filter(item => {
      if (tab === 'pending') {
        // Show appointments for selected date OR any pending patient requests from the database
        return item.dateKey === selectedDate || !item.done;
      }
  ...
  const visible = dayAppointments
    .filter(item => tab === 'done' ? item.done : !item.done)
    .sort((a, b) => {
...
function AppointmentCard({ item, tab, onPress, onPrescribe, selectedDate }: { item: Appointment; tab: 'pending' | 'done'; onPress: () => void; onPrescribe: () => void; selectedDate?: string }) { return <Pressable onPress={onPress} style={s.card}><View style={s.time}><Text style={s.timeText}>{item.time}</Text></View><View style={s.cardInfo}><Text style={s.name}>{item.patientName}</Text><Text style={s.meta}>Age {item.age}  •  {item.visit}{item.dateKey && selectedDate && item.dateKey !== selectedDate ? `  •  ${formatDate(item.dateKey)}` : ''}</Text>{tab === 'done' && <View style={[s.priorityBadge, { backgroundColor: `${C[item.priority]}18` }]}><Text style={[s.priorityBadgeText, { color: C[item.priority] }]}>{item.priority} priority</Text></View>}</View>{tab === 'done' ? <Pressable onPress={onPrescribe} style={s.rxButton}><Ionicons name="document-text-outline" size={17} color={C.blue} /><Text style={s.rxText}>Prescribe</Text></Pressable> : <Ionicons name="chevron-forward" size={19} color={C.muted} />}</Pressable>; }
