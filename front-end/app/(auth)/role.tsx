...
const BLUE = '#1A66E8'; // logo, same as the other auth screens
const NAVY = '#122745'; // headline
const MUTED = '#5B6B7F'; // body copy

/** The value we store on the Clerk user. */
type Role = 'doctor' | 'patient' | 'pharmacist';

/** The faint dot grids in the corners of the design. */
...
export default function RoleScreen() {
  ...
  /**
   * Stores the chosen role on the Clerk user, then moves on.
   *
   * All three cards do these same two things, so they share this and pass in
   * only what differs: the value to save and where to go afterwards.
   */
  const chooseRole = async (role: Role, destination: Href) => {
    ...
    try {
      // updateMetadata merges instead of overwriting, so the licence number
      // added later will sit alongside the role rather than replacing it.
      //
      // Waiting for it to finish before navigating matters: the next screen
      // reads this value, and without the await it could arrive first.
  ...
  // Each role keeps its own destination: a form that collects that role's
  // details. Where they go after submitting is decided by those screens.
  ...
  return (
    <View style={styles.screen}>
      {/* Decoration. Sits behind everything and is ignored by touches, so a
          card near the edge still receives its press. */}
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        >
          {/* Logo, same treatment as login and signup */}
